...
int main(int argc, char *argv[])
{
    ...
    int key_press;
    SDL_WM_SetIcon(IMG_Load("ArcadeEater_16X16.png"), NULL);
    SDL_WM_SetCaption("ArcadeEater","ArcadeEater_16X16.png");
...
void ArcadeEater_sprites(SDL_Rect location,int direction,SDL_Surface* screen,char comp[20][12])
{
...
// end def of sprites
    SDL_Surface *one,*two;
    one=load_image("ArcadeEater.gif");
    two=load_image("ArcadeEater_follow.gif");
...
void gameplay(SDL_Surface* screen)
{
    ...
    while (quit==no)
    {
           while(SDL_PollEvent(&event))
           {
            ...
            ArcadeEater_sprites(cor,key_press,screen,comp);
